...
- 多行命令（命令本身含换行、`\` 续行）下光标不在 prompt 最后一行的场景——按已知限制处理，v1 不支持，留待后续研究。
  - 可行性结论：光标所在"逻辑行"的渲染可做（把命令按 `\n` 切行，`nl_before` 个换行决定光标在第几行；prompt 最后一行仅在 `nl_before==0` 时拼到行首），且对单行命令是无害推广。
  - 待验证的风险点：视口 2 行（光标行 + 候选行）下，光标正下方若有命令行，ratatui `Inline` 视口是否会把它干净往下推（而非覆盖）。从残影 bug 推断光标行本身是被直接画上的，故对此无把握。后续实现前先写小测试判定。
